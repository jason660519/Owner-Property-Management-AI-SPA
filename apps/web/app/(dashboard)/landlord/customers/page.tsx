...
export default function LandlordCustomersPage() {
  ...
  const updateSelectedCustomer = async (next: Partial<Customer>) => {
    ...
    const res = await fetch(`/api/landlord/customers/${selectedCustomer.id}`, {
      method: 'PUT',
    ...
    if (!res.ok) {
      throw new Error('Update selected customer failed')
    }

    setCustomers((prev) => prev.map((item) => {
      if (item.id !== selectedCustomer.id) return item
      return {
        ...item,
        ...next,
