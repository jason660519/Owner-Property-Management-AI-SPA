...
const SYSTEM_NOTICES = [
  '系統通知：新訊息將同步顯示於此，並可透過帳號設定開啟 Email 通知。',
  '系統通知：訊息紀錄可依日期篩選，租約期間內之紀錄將保留供查閱。',
]
...
export default function TenantCommunicationCenterPage() {
  ...
  const { data: messages = [], isLoading } = useQuery({
    queryKey: ['tenant-communication-messages'],
  ...
  const markAsReadMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tenant-communication-messages'] })
    },
  })
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      ...
      return messageService.sendMessage({
        ...
        subject: `Re: ${selectedMessage.subject ?? '租客溝通中心'}`,
        content: content || '已傳送附件，請查收。',
        message_type: attachmentNames.length > 0 ? 'file' : 'text',
        attachment_urls: attachmentNames.map((name) => `mock://tenant-attachments/${name}`),
      })
    },
    onSuccess: () => {
      setComposeText('')
  ...
  return (
    <DashboardLayout
      ...
      breadcrumbs={[
        ...
        { label: '簽約儀表板', href: '/tenant/contracted/dashboard' },
        { label: '溝通中心' },
      ]}
      greeting="與房東即時溝通，查看已讀回條並搜尋歷史訊息。"
    >
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    {selectedMessage.attachment_urls.map((url) => (
                      <div key={url} className="flex items-center gap-2 text-sm text-accent">
                        {url.endsWith('.pdf') ? <FileText className="h-4 w-4" /> : <ImageIcon className="h-4 w-4" />}
