...
const RAW_FEATURES: RoadmapFeature[] = [
  ...
  // --- Row 146: 尋人資料庫 — 工作區整合 + 強制 dataset + Scope 顏色標記 ---
  {
    ...
    testProgress:
      "Step 1+2+3+4 共 30 cases pass：page.test.tsx ×6（tab dispatcher）+ tools/page.test.tsx ×2 + import/__tests__/dataset-root-required.test.tsx ×5（UI 必填）+ submit/__tests__/dataset-root-required.test.ts ×3（API 400）+ jobs/__tests__/dataset-root-required.test.ts ×3（API 400 + 202）+ lib/people-db/__tests__/dataset-color.test.ts ×7（HSL hash 純函式）+ search/__tests__/scope-and-color.test.tsx ×4（scope banner + DatasetBadge mount）。整個 people-db 套件 295/295 全綠（+48 vs Sprint 6 的 247）。e2e 待後續 PR。",
    lastModifiedBy: "Claude Sonnet 4.5",
    lastModifiedDate: "2026/04/19",
  },
];
