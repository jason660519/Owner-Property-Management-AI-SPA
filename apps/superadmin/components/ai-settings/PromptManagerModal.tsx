...
export function PromptManagerModal({
  ...
  const panelHeight =
    variant === 'modal' ? 'min(88vh, 760px)' : 'min(calc(100vh - 260px), 860px)';
  ...
  if (variant === 'page') {
    return <div className="w-full flex justify-center px-6 py-6">{panel}</div>;
  }
  ...
  return createPortal(
    <>
      ...
      >
        <div style={{ pointerEvents: 'auto' }} className="w-full flex justify-center">
          {panel}
        </div>
      </div>
    </>,
