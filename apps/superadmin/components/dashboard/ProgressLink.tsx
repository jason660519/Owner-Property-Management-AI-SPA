...
export function ProgressLink({ link, className = '' }: { link: ProgressLinkType; className?: string }) {
  ...
  return (
    <Link
      href={href}
      className={`group flex items-center justify-between px-3 py-2 rounded-md text-sm text-text-secondary hover:bg-bg-tertiary hover:text-text-primary transition-all ${className}`}
    >
