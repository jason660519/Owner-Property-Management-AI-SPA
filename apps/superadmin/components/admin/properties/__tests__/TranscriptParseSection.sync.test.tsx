...
describe('TranscriptParseSection — 解析設定與 OCR 設定同步', () => {
  it('displays parser models from online_ocr_parse assigned_models (single source of truth)', () => {
  ...
  it('when opening 解析設定 panel, calls refreshSilent so list syncs with OCR settings page', () => {
  ...
  it('after refreshSilent returns updated modules, parser list shows new assigned_models', () => {
    ...
    const { rerender } = render(<TranscriptParseSection transcriptDocs={[]} />);
    fireEvent.click(screen.getByRole('button', { name: /解析設定/i }));
    ...
    const lists = screen.getAllByRole('list');
    const parserList = lists.find((ul) => ul.className.includes('max-h-40')) ?? lists[0];
    expect(parserList.querySelectorAll('li')).toHaveLength(3);
    expect(screen.getByText(/google \/ gemini-2.0-flash/)).toBeInTheDocument();
  });

  it('shows link to OCR tab for single source of truth', () => {
    render(<TranscriptParseSection transcriptDocs={[]} />);
    fireEvent.click(screen.getByRole('button', { name: /解析設定/i }));
