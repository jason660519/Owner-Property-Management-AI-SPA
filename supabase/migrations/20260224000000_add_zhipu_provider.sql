-- Add 'zhipu' to all provider check constraints
-- Affects: ai_api_keys, ai_model_selections, ai_chat_logs, ai_model_evaluations, ai_key_validation_cache
...
ALTER TABLE public.ai_api_keys
  ...
  CHECK (provider IN ('openai', 'anthropic', 'gemini', 'deepseek', 'grok', 'zhipu'));
...
-- ── ai_chat_logs (provider column added later, constraint may differ) ─────────
...
ALTER TABLE public.ai_chat_logs
  ...
  CHECK (provider IS NULL OR provider IN ('openai', 'anthropic', 'gemini', 'deepseek', 'grok', 'zhipu'));
