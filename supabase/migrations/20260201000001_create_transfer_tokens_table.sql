...
-- Create transfer_tokens table (explicit public schema for consistency)
...
CREATE INDEX idx_transfer_tokens_token ON public.transfer_tokens(token);
CREATE INDEX idx_transfer_tokens_user_id ON public.transfer_tokens(user_id);
CREATE INDEX idx_transfer_tokens_expires_at ON public.transfer_tokens(expires_at);
...
-- Add comment
COMMENT ON TABLE public.transfer_tokens IS 'Stores one-time use transfer tokens for secure session transfer between Next.js and Expo apps';
