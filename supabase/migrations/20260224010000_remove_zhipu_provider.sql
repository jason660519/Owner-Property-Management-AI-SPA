-- Remove 'zhipu' from all provider check constraints and delete zhipu data

-- ── Delete zhipu data ─────────────────────────────────────────────────────────
DELETE FROM public.ai_key_validation_cache WHERE provider = 'zhipu';
DELETE FROM public.ai_model_selections      WHERE provider = 'zhipu';
DELETE FROM public.ai_model_evaluations     WHERE provider = 'zhipu';
DELETE FROM public.ai_api_keys              WHERE provider = 'zhipu';

-- ── Revert check constraints (remove 'zhipu') ─────────────────────────────────
...
ALTER TABLE public.ai_chat_logs
  ...
  CHECK (provider IS NULL OR provider IN ('openai', 'anthropic', 'gemini', 'deepseek', 'grok'));
