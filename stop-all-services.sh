...
# 停止開發進度追蹤系統
echo -e "${YELLOW}2️⃣  停止開發進度追蹤系統 (Port 3001)...${NC}"
...
# 停止 OCR 服務
echo -e "${YELLOW}3️⃣  停止離線謄本查詢系統 (Port 8000)...${NC}"
...
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo -e "${YELLOW}4️⃣  停止 Supabase...${NC}"
    supabase stop
...
# 清理臨時日誌
echo -e "${YELLOW}5️⃣  清理臨時日誌...${NC}"
rm -f /tmp/nextjs.log /tmp/ocr_service.log 2>/dev/null || true
