...
- **進度報告**：
  - OCR 相關進度 → `docs/progress-reports/OCR開發進度報告/`
  - 專案規劃與 Roadmap → `docs/progress-reports/roadmap/`
  - 每日工作報告 → `docs/progress-reports/工程師每日工作報告/`
  - 資料庫相關進度 → `docs/progress-reports/資料庫進度報告/`
  - 其他進度報告 → `docs/progress-reports/`
- **產品需求與使用場景**：存入 `docs/產品概述及使用場景說明/`
- **技術選型與架構文件**：存入 `docs/硬體與軟體技術選型說明/`
...
根目錄僅保留以下各類型的核心檔案，其餘一律歸檔至 `docs/` 下對應目錄：

- **環境設定**：`.env.*`, `.gitignore`, `.editorconfig`
- **依賴管理**：`package.json`, `pnpm-workspace.yaml`, `turbo.json`
- **項目入口**：`README.md`, `CONTRIBUTING.md`, `LICENSE`
- **AI 協作規範**：`CLAUDE.md`, `FILE_CREATION_CHECKLIST.md`
- **工具配置**：`tsconfig.json`, `eslint.config.js`
- **快速啟動指南**：`docs/deployment-guides/本案開發環境快速啟動指南.md`
...
| 日期       | 版本 | 修改者                 | 修改內容                                                                |
| ---------- | ---- | ---------------------- | ----------------------------------------------------------------------- |
| 2026-02-01 | 2.2  | Gemini 3 Pro (Preview) | 更新文件架構規範，反映實際 docs/ 目錄結構；更新歸檔流程與根目錄淨空規則 |
| 2026-01-30 | 2.1  | Claude Sonnet 4.5      | 新增 AI 協作者識別規範與修改歷史追蹤機制                                |
