...
本專案採用 **Turborepo** 進行 Monorepo 管理，區分為對外網站 (Web) 與管理應用 (App) 雙軌並行開發。

#### Monorepo 架構 (Turborepo)
*   **apps/web**: Next.js 對外官網與行銷頁面
*   **apps/mobile**: Expo 房東管理 App 與租客 App
*   **packages/ui**: 共用 UI 元件庫
*   **packages/utils**: 共用工具函式庫
*   **packages/types**: 共用 TypeScript 型別定義

#### 1. 對外網站平台 (Public Web Platform)
*   **框架**: **Next.js 15** (React 19)
*   **定位**: 租客註冊、物件搜尋列表、SEO 行銷頁面、部落格系統。
*   **渲染策略**: 
    *   **ISR (Incremental Static Regeneration)**: 用於物件詳情頁與部落格文章，兼顧 SEO 與效能。
    *   **SSR (Server-Side Rendering)**: 用於動態搜尋結果與會員中心。
*   **部署**: Vercel 或 Docker 容器化部署。
...
#### 3. 後端服務 (Backend as a Service)
*   **核心後端**: **Supabase** 
    *   **PostgreSQL 資料庫**:
        *   儲存 JSONB 格式的文檔資料（如 權狀謄本PDF 解析結果）。
        *   管理關聯式資料（物件、租約、財務紀錄）。
...
*   **任務管理**: **Vibe Kanban**
*   **建置系統**: **Turborepo**
    *   **用途**: 高效管理 Monorepo，支援增量建置 (Incremental Build) 與遠端快取 (Remote Caching)。
    *   **策略**: 統一管理 `apps/web` 與 `apps/mobile` 的依賴與建置流程。

---

## 3. 硬體架構選型 (Hardware Architecture)

因預算有限，本專案前期規劃自建伺服器（Self-hosted Server）作為地端或私有雲部署選項。
...
### 3.2 伺服器用途與部署規劃
1.  存放前後端必要文件，圖片，檔案，docker containers，管理工具...等．
2.  **私有化後端託管**:
