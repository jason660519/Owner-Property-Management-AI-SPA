...
儲存房屋基本資料與謄本解析結果。目前 Supabase 初始版本對應欄位如下，後續擴充以 transcript_data JSONB 承載額外資訊。

| 欄位名稱        | 資料型態    | 必填 | 預設值            | 說明                          |
| --------------- | ----------- | :--: | ----------------- | ----------------------------- |
| id              | UUID        |  ✓   | gen_random_uuid() | 主鍵                          |
| agent_id        | UUID        |  ✓   | -                 | 物件負責仲介，FK → auth.users |
| address         | TEXT        |  ✓   | -                 | 物件完整地址                  |
| district        | TEXT        |  -   | -                 | 行政區                        |
| total_area      | NUMERIC     |  -   | -                 | 建物總面積（平方公尺）        |
| building_age    | INTEGER     |  -   | -                 | 屋齡（年）                    |
| transcript_data | JSONB       |  -   | '{}'              | Jason JSON，儲存謄本解析結果  |
| created_at      | TIMESTAMPTZ |  ✓   | NOW()             | 建立時間                      |
| updated_at      | TIMESTAMPTZ |  ✓   | NOW()             | 更新時間                      |

> 後續擴充建議：於 transcript_data 中保留原始謄本欄位，避免頻繁調整 Schema；若需常用索引，可再拆出獨立欄位（例如 registration_date、land_parcel_no）。

#### transcript_data Jason Schema 範例

```json
{
  "metadata": {
    "document_id": "a8f5f167-...",
...
#### ocr_result Jason Schema 重點

- `metadata.document_id`：對應 property_documents.id，便於索引。
- `sections`：沿用 properties.transcript_data 結構，以確保跨表一致性。
- `post_process`：紀錄欄位修正、人工覆寫與審核紀錄，例如 `{ "operator": "user_123", "patched_fields": ["ownerships[0].address"] }`。
- `attachments`：若拆分頁面輸出圖檔，紀錄 Supabase Storage 路徑與頁碼，方便除錯。
